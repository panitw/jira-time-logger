...
- **FR6:** Worker can log time against any Jira **subtask** they have permission to log against. Worklogs against tasks/stories/Epics directly are not supported — this enforces the org-wide agreement that all time is recorded at the subtask level.
  - **AMENDED 2026-07-27 (owner decisions D-7.4-11 and D-CA-1).** The "not supported" clause no longer holds. Search shows **every** issue type and permits logging directly to a Task/Story/Epic, with a visible amber warning that those hours may not reach the approval matrix (D-7.4-11). The catch-all picker accepts **Sub-task and Task** (D-CA-1), because a real catch-all project built from Tasks made the original filter return zero rows — leaving time off permanently unconfigurable and silently breaking FR11. Subtask-level logging remains the *preferred* path, and the rollup consequence is real: `lib/hierarchy.ts` walks subtask → parent → epic, so hours on a Task do not enter Epic 5's matrix. It is no longer a *prohibition*.
...
- **FR10:** Worker can log time against subtasks in the designated **catch-all project** (default project key: `KNP`). The catch-all picker presents a flat list of pre-existing shared subtasks (e.g., "Meetings", "Support Production Incidents") configured at the project level — workers do not create their own catch-all subtasks.
  - **AMENDED 2026-07-27 (D-CA-1).** Read "shared **items**", not "shared subtasks": the picker queries `issuetype IN ("Sub-task", "Task")`. A filter is deliberately retained rather than dropped, so the catch-all stays the curated shared list this FR describes instead of every issue in the project. The "workers do not create their own" rule is unchanged. User-facing copy accordingly says "time-off **ticket**" and "catch-all **items**" — naming the issue type in copy would assert something false for a Task-based project.
