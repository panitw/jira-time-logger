...
- **CLOSED by Story 7.9.** ~~Three copies of the chrome progress-bar quantisation logic, one shared helper needed.~~ `ChromeHeader.tsx`, `WeekChromeHeader.tsx`, and `DayStatusIndicator.tsx`'s `stacked` branch each carried their own `pctToWidthClass`/width-table pair — one of them (`ChromeHeader.tsx`) still had the `Math.round` quantisation defect (97.6% → `w-full`, 2.4% → `w-0`). Story 7.9 (Obligation 1) deleted all three private copies and migrated their four call sites onto `lib/progress-width.ts` (created by Story 7.8, D-7.8-19a). Zero copies remain, pinned by `lib/progress-width.grep.test.ts`. [components/shell/ChromeHeader.tsx, components/week/WeekChromeHeader.tsx, components/shared/DayStatusIndicator.tsx, lib/progress-width.ts]
